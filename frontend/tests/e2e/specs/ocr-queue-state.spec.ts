...
test.describe('OCR Queue Popover State Persistence & Intelligent Dismissal', () => {
    ...
    test('should maintain open state when clicking "Related Actions" (Intelligent Keep-Open)', async ({ page }) => {
        ...
        const item1 = page.locator('[data-testid^="page-item-"]').nth(0);
        await item1.hover();
        await waitForHealthyService(page);
        await item1.getByTestId('scan-page-btn').click();
        await page.getByTestId('ocr-queue-badge').waitFor({ state: 'visible' });

        // 4. Manually trigger popover OPEN
        await page.getByTestId('ocr-queue-badge').click();
        await page.getByTestId('ocr-queue-popover').waitFor({ state: 'visible' });

        // 5. Click the "Recognize" button of the SECOND item (Outside popover, but whitelisted)
        const item2 = page.locator('[data-testid^="page-item-"]').nth(1);
        await item2.hover();
        ...
        expect(await page.getByTestId('ocr-queue-popover').isVisible()).toBeTruthy();
    });

    test('should maintain open state when interacting with Page Viewer actions', async ({ page }) => {
        // 1. Mock OCR
        await apiMocks.mockOCR({ delay: 5000 });
        ...
        const viewerOCRBtn = page.getByTestId('ocr-trigger-btn');
    ...
    test('should close when clicking outside on non-related elements', async ({ page }) => {
        // 1. Mock OCR
        await apiMocks.mockOCR({ delay: 5000 });
        ...
        await expect(page.getByTestId('ocr-queue-popover')).toBeHidden();
    });
});
